...
def sub():
    try:
        res = request.args
        print(res)
        nameId = res.get("nameId")
        commitTime = res.get("time")
        # payload = request.json
        # nameId = payload.get("name")
        # commitTime = payload.get("time")
        data = dbSession.query(Entry).filter(Entry.name == nameId , Entry.time == commitTime).one()
        data.confirmNum = data.confirmNum - 1
        dbSession.add(data)
        dbSession.commit()
        if data.confirmNum <= 0:
            auth = tweepy.OAuthHandler(CONSUMER_KEY, CONSUMER_SECRET)
...
def addDataToDatabase(unix, name, message, count, type, access_token, access_token_secret, ws):
    # print("data: ",data)
    entry = Entry(time = str(unix), name=str(name), message=str(message), confirmNum=str(count), confirmType=str(type), access_token=str(access_token), access_token_secret=str(access_token_secret))
    dbSession.add(entry)
    try:
        dbSession.commit()
        ws.send("?time="+str(unix)+"&name="+str(name))
...
def selectDataToDatabase():
    ...
    time = res.get("time")
    print(name)
    answer = dbSession.query(Entry).filter(Entry.name == name , Entry.time == time).one()
    auth = tweepy.OAuthHandler(CONSUMER_KEY, CONSUMER_SECRET)
    auth.set_access_token(answer.access_token, answer.access_token_secret)
    api = tweepy.API(auth)
    userName = api.get_user(answer.name)
    print(userName.name)
    return jsonify({'name': userName.name, "message": answer.message})
